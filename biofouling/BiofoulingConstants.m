classdef BiofoulingConstants
    ...
    properties (Constant)
        ...
        %%% growth
        mu_max = 1.85 / seconds(days(1)); %  max growth rate algae (s^-1)
        alpha = .12 / seconds(days(1));  % initial slope growth rate algae (s^-1)
        I_opt = 1.75392e13 / seconds(days(1)); % optimal light intensity algae growth (micro mol quanta m^-2 s^-1)
        T_min = .2;   % min temp for growth (Celsius)
        T_max = 33.3;   % max temp for growth (Celsius)
        T_opt = 26.7;   % optimal temp for growth (Celsius)
        %%% mortality
        m_A = .39 / seconds(days(1));  % mortality rate (s^-1)
        %%% respiration
        Q_10 = 2; % Temperature dependence coefficient respiration (unitless)
        Resp_A = .1 / seconds(days(1)); % Respiration rate (s^-1)
    end
end
